...
- [x] `name`：持久化类的标识属性的名字
- [x] `column`：数据库表的主键这段的名字
- `<generator>`设置主键生成方式
- [x] 该元素的作用是指定主键的生成器，通过一个class属性指定生成器对应的类。(通常与<id>元素结合使用)，--native是Hibernate主键生成器的实现算法之一，由Hibernate根据底层数据库自行判断采用identity、hilo、sequence其中一种作为主键生成方式。
- `<property>`定义属性
- [x] 用于持久化类的属性与数据库表字段之间的映射,包含如下属性：
- [x] name：持久化类的属性名，以小写字母开头
- [x] column：数据库表的字段名
- 