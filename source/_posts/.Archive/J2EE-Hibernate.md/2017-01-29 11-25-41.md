...
> `Hibernate`使用`OID`(对象标识符)来标识对象的唯一性，`OID`是关系数据库中主键在`Java`对象模型中的等价物,在运行时,`Hibernate`根据`OID`来维持`Java`对象和数据库表中记录的对应关系
 ...
 - `<generator>`设置主键生成方式
   ...
   5. `uuid.hex`：用一个128-bit的UUID算法生成32位字符串类型的标识符。
        - `native`  :根据底层数据库的能力选择identity, sequence 或者hilo中的一个。<font color='red'>推荐用这个</font>
   6. `assigned`：让应用程序在save()之前为对象分配一个标示符。 
   7. `foreign`：:使用另外一个相关联的对象的标识符。和<one-to-one>联合一起使用。
   8. `详情：`:[Hibernate generator(http://blog.csdn.net/ye1992/article/details/19632001)
