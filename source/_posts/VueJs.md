...
```
> `v-model=name`指从`data`中去的`name`的值，<font color='#80D63F'>**相当于value='HHH',但如果`v-model`和value同时存在,`v-model`会覆盖value的值，因为`v-model`为动态绑定！！**</font>
> 如果`type=hidden`了:
...
> v-model貌似不能访问json对象，例如`v-model='message.value'`
...
``` html
<!-- :和v-bind等同 -->
<input type="text" v-model="message" :name="message" value="asd"/>